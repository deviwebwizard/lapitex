...
export function ProductGallery({ images, alt, isSoldOut }: { images: string[]; alt: string; isSoldOut?: boolean }) {
  ...
  return (
    <>
      <div className="flex-grow flex items-center justify-center bg-gray-50 rounded-[2rem] relative min-h-[300px] md:min-h-[450px] overflow-hidden border border-gray-100 shadow-inner group">
        ...
        {isSoldOut && <SoldOutBanner className="py-2.5 text-sm md:text-base" />}
        <button type="button" onClick={showPrevious} aria-label="Previous product image" className="absolute left-4 top-1/2 -translate-y-1/2 z-30 rounded-full bg-white/85 p-2 text-gray-700 shadow-md hover:bg-white transition-colors">
          <ChevronLeft className="h-5 w-5" />
        </button>
        <button type="button" onClick={showNext} aria-label="Next product image" className="absolute right-4 top-1/2 -translate-y-1/2 z-30 rounded-full bg-white/85 p-2 text-gray-700 shadow-md hover:bg-white transition-colors">
          <ChevronRight className="h-5 w-5" />
        </button>
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 flex gap-2 rounded-xl bg-black/20 p-2 backdrop-blur-sm">
          {images.map((image, index) => (
